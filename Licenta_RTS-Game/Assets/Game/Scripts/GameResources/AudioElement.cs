...
            GameObjectsList list = MonoBehaviour.FindObjectOfType(typeof(GameObjectsList)) as GameObjectsList;
    ...
    public void Add(List<AudioClip> sounds, List<float> volumes)
    {
        for (int i = 0; i < sounds.Count; i++)
        {
            ...
            temp.GetComponent<AudioSource>().spatialBlend = 1;
            temp.GetComponent<AudioSource>().maxDistance = 125;
            temp.GetComponent<AudioSource>().rolloffMode = AudioRolloffMode.Linear;
            temp.GetComponent<AudioSource>().dopplerLevel = 0;
